...
for j in ( 0.001 , 0.00216 , 0.00464 , 0.01 , 0.0216 , 0.0464 , 0.1 , 0.216 , 0.464 ,  1 , 2.16, 4.64 , 10):
    ...
    classifier =OneVsRestClassifier( LogisticRegression(penalty = 'l2', C = j ,  solver = 'liblinear'))
...
plt.plot( (1 / np.array(( 0.001 , 0.00216 , 0.00464 , 0.01 , 0.0216 , 0.0464 , 0.1 , 0.216 , 0.464 ,  1 , 2.16, 4.64 , 10)) ) , (np.array(scores_LR_l2_MASK)) )
plt.xlabel('The regulization parameter')
plt.ylabel('Success rate of the pipeline.')      
plt.xscale('log')        
plt.title("Success Rate vs Regularization Parameter for Ventral Temporal\nCortex Mask and Logistic Regression with Penalty l2 Norm Classifier")
plt.grid()
