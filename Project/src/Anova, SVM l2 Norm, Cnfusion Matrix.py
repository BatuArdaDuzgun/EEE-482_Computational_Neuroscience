...
rfe_svc = OneVsRestClassifier( Pipeline([('Anova', feature_selection), ('svc', LinearSVC(penalty='l2', C = 0.0216 , dual=False))]) )
fmri_masked = haxby['sub-2']['data']['1_fmrimasked']
cv = LeaveOneGroupOut()
...
im = ax.imshow(Confused_ANOVA_SVM_l2, cmap='YlGnBu' )
...
for i in range(8):
    for j in range(8):
        if Confused_ANOVA_SVM_l2[i, j] > 50:
            text = ax.text(j, i, Confused_ANOVA_SVM_l2[i, j], ha="center", va="center", color="w")
        else:
            text = ax.text(j, i, Confused_ANOVA_SVM_l2[i, j], ha="center", va="center", color="k")
