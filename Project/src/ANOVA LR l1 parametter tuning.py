...
"""
Anova, LR l1, Regularization Parameter
"""
...
for j in ( 0.0216 , 0.0464 , 0.1 , 0.216 , 0.464 ,  1 , 2.16, 4.64 , 10 , 21.6, 46.4 , 100):
    ...
    rfe_svc = OneVsRestClassifier( Pipeline([('Anova', feature_selection), ('lr', LogisticRegression(penalty = 'l1', C = j ,  solver = 'liblinear'))]) )
    fmri_masked = haxby['sub-2']['data']['1_fmrimasked']
    cv = LeaveOneGroupOut()
    ...
    scores_LR_l1_ANOVA.append( succes_count / 7.68)



plt.figure(figsize=(8, 5))    
plt.plot( (1 / np.array(( 0.0216 , 0.0464 , 0.1 , 0.216 , 0.464 ,  1 , 2.16, 4.64 , 10 , 21.6, 46.4 , 100 )) ) , (np.array(scores_LR_l1_ANOVA)) )
plt.xlabel('The regulization parameter')
plt.ylabel('Success rate of the pipeline.')      
plt.xscale('log')        
plt.title("Success Rate vs Regularization Parameter for ANOVA \n and Logistic Regression with Penalty l1 Norm Classifier")
plt.grid()
