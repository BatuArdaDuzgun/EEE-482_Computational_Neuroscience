...
for j in (0.007, 0.01, 0.03, 0.07, 0.1 , 0.3 ):
    ...
    rfe_svc = OneVsRestClassifier( Pipeline([('rfe', feature_selection), ('svc', LinearSVC(penalty='l2', C=j , dual=False))]) )
    fmri_masked = haxby['sub-2']['data']['1_fmrimasked']
    cv = LeaveOneGroupOut()
    ...
    rfe_svc.fit(fmri_masked2[(session_label2 != 9) | (session_label2 != 10) | (session_label2 != 11)] , conditions2[(session_label2 != 9) | (session_label2 != 10) | (session_label2 != 11)] )

    y_pred_ovo = rfe_svc.predict(fmri_masked2[(session_label2 == 9) | (session_label2 == 10) | (session_label2 == 11)])

    temp_matrix = confusion_matrix(y_pred_ovo, conditions2[(session_label2 == 9) | (session_label2 == 10) | (session_label2 == 11)])
    ...
    scores_RFE_l2.append( succes_count / 7.68 * 4)
...
plt.title("Confusion Matrix for RFE Preprocessor \n and SVM with Penalty l2 Norm Classifier")
